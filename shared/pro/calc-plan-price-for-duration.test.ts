...
const plan = { price: 4, price_stars: 250 } as PlanDb;

test("calc plan price $", () => {
  expect(calcPlanPriceForDuration(PaymentMethodType.Usd, plan, 1)).toBe(
    plan.price,
  );
  expect(calcPlanPriceForDuration(PaymentMethodType.Usd, plan, 6)).toBe(16);
  expect(calcPlanPriceForDuration(PaymentMethodType.Usd, plan, 12)).toBe(24);
});
